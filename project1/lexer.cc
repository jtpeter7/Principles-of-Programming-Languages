...
Token LexicalAnalyzer::ScanNumber()
{
    ...
    if (isdigit(c)) {
		...
		tmp.token_type = NumType().token_type;
        //tmp.token_type = NUM;
        tmp.line_no = line_no;
        return tmp;
    } else {
...
Token LexicalAnalyzer::NumType() {
	Token temp;
	string Value = tmp.lexeme;
	bool real = true, b8 = true, b16 = true;
	char ValueAsChar[Value.size() + 1];
	...
	if (int(ValueAsChar[0]) != 48) {//IF positive digit
		for (int i = 0; i < Value.length(); i++) {
			if (real) {
				...
				else if ((int(ValueAsChar[i]) == 120) && int(ValueAsChar[i + 1]) == 48) {
					b16 = false;
					real = false;
				}
				else {
				...
				else if ((int(ValueAsChar[i]) == 120) && int(ValueAsChar[i + 1]) == 49) {
					b8 = false;
					real = false;
				}
				else {
